...
class SimpleEnemyEnv(MiniGridEnv):
    ...
    def __init__(self, max_steps=500, agent_pos=None, goal_pos=None, num_obstacles=10, seed=1327):
        self._agent_default_pos = agent_pos
        self._goal_default_pos = goal_pos
        self.door_color = COLOR_NAMES[1]
        self.n_obstacles = num_obstacles
        super().__init__(grid_size=9, max_steps=max_steps, seed=seed)
        self.action_space = gym.spaces.Discrete(4)
        self.observation_space = gym.spaces.Box(low=np.zeros(98), 
                                                high=np.ones(98))
    ...
    def convert_obs(self, obs):
        ...
        return np.hstack((obstacles, doors)).flatten()
